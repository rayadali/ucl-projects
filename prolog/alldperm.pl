...
checkRange(X,Old,New):- range(X,Old,Old,New);
						Perm=Old,perm(Perm,P),checkRange(X,P,New).
...
range(X,[H|T],Old,New):-		isMember(H,Old)->del(X,Old,New),assignTo1(X,A).
range(X,[H|T],Old,New):-		range(X,T,Old,New).
